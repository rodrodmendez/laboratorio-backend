...
export async function listContacts(customerId = '') {
  const dbResult = await query(`
    select contact_id, customer_id, contact_name, email, phone, is_primary, contact_origin, contact_role
    from lab.contact_person
    where ($1 = '' or customer_id = $1)
    order by is_primary desc, contact_name
  `, [customerId]).catch(() => query(`
...
export async function saveContact(contact) {
  const payload = {
    ...
    contactOrigin: String(contact.contactOrigin || (contact.isPrimary ? 'primary' : 'manual')).trim(),
  ...
  const dbResult = await query(`
    insert into lab.contact_person (contact_id, customer_id, contact_name, email, phone, is_primary, contact_origin, contact_role)
    ...
    on conflict (contact_id) do update set
      ...
      updated_at = now()
    returning contact_id, customer_id, contact_name, email, phone, is_primary, contact_origin, contact_role
  `, [
    payload.contactId,
    payload.customerId,
    payload.contactName,
  ...
  const next = {
    ...
    contactOrigin: payload.contactOrigin,
